...
ChatGrpcClient::ChatGrpcClient() {
  ...
  for (auto &word : words) {
    ...
    std::transform(name_key.begin(), name_key.end(), name_key.begin(),
                   ::tolower);
...
TextChatMsgRsp ChatGrpcClient::NotifyTextChatMsg(std::string server_ip,
  ...
  if (find_iter == _pools.end()) {
    std::cout << "[TextChat][gRPC][Client] pool not found for key=" << key
              << " pools=";
    for (auto &kv : _pools)
      std::cout << kv.first << ' ';
    std::cout << std::endl;
  ...
  if (!status.ok()) {
    std::cout << "[TextChat][gRPC][Client] rpc failed ok=false error_code="
              << status.error_code()
