...
// 
// 作用：
//   负责读取和解析config.ini配置文件，将配置信息加载到内存中的配置映射表
// 
// 实现逻辑：
//   1. 获取当前可执行文件的路径
...
ConfigMgr::ConfigMgr()
{
    ...
    for (const auto& section_pair : pt) {
        ...
        for (const auto& key_value_pair : section_tree) {
            const std::string& key = key_value_pair.first;  // 配置项名称
            const std::string& value = key_value_pair.second.get_value<std::string>();  // 配置项值
        ...
        // 将section和其配置数据存储到配置映射表
        _config_map[section_name] = sectionInfo;

        // 打印所有section的key-value（用于调试）
