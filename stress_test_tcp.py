...
"""
TCP 吞吐量压力测试脚本
测试 ChatServer 的消息处理能力

用法：
    python3 stress_test_tcp.py --host 192.168.132.130 --port 8090 --connections 100 --duration 30
"""
...
class TCPStressTest:
    ...
    def _build_tcp_packet(self, msg_id, body):
        """构造 ChatServer TCP 协议包
    ...
    def _maintain_connection(self, sock, socket_id):
        """维持连接并发送合法的登录请求"""
        try:
            # 第一步：发送登录请求（msg_id=1005）
            login_body = json.dumps({
                "uid": 1000 + socket_id,  # 模拟不同的用户ID
                "token": f"test_token_{socket_id}"
            })
            login_packet = self._build_tcp_packet(1005, login_body)
            sock.sendall(login_packet)
            with self.lock:
                self.stats['messages_sent'] += 1
                self.stats['bytes_sent'] += len(login_packet)
            
            # 第二步：持续发送心跳/测试消息
            while time.time() - self.start_time < self.duration:
                try:
                    # 发送离线消息请求（msg_id=1023）作为心跳
                    heartbeat_body = json.dumps({
                        "uid": 1000 + socket_id
                    })
                    heartbeat_packet = self._build_tcp_packet(1023, heartbeat_body)
                    sock.sendall(heartbeat_packet)
                    with self.lock:
                        self.stats['messages_sent'] += 1
                        self.stats['bytes_sent'] += len(heartbeat_packet)
    ...
    def _print_stats(self):
        ...
        print(f"\n📈 性能指标:")
        if elapsed > 0 and self.stats['connected'] > 0:
        ...
        elif self.stats['connected'] == 0:
            print(f"  ⚠️  无成功连接，无法计算性能指标")
        
        print(f"\n� 消息吞吐量:")
        print(f"  发送消息数: {self.stats['messages_sent']:,}")
        print(f"  发送字节数: {self.stats['bytes_sent']:,} bytes ({self.stats['bytes_sent']/1024:.2f} KB)")
        print(f"  接收消息数: {self.stats['messages_received']:,}")
        print(f"  接收字节数: {self.stats['bytes_received']:,} bytes ({self.stats['bytes_received']/1024:.2f} KB)")
        
        if elapsed > 0:
            msg_per_sec = self.stats['messages_sent'] / elapsed
            bytes_per_sec = self.stats['bytes_sent'] / elapsed
            print(f"\n  📊 发送吞吐量: {msg_per_sec:,.1f} msg/s")
            print(f"  📊 发送带宽: {bytes_per_sec/1024:.2f} KB/s ({bytes_per_sec/1024/1024:.2f} MB/s)")
            
            if self.stats['connected'] > 0:
                print(f"  📊 每连接平均: {msg_per_sec/self.stats['connected']:.1f} msg/s")
...
def main():
    ...
    parser.add_argument('--interval', type=float, default=0.1, help='消息发送间隔秒数 (默认: 0.1 = 10 msg/s/conn)')
