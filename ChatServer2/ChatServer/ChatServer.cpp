...
int main() {
  ...
  std::cout << "trying to load config at: "
            << (std::filesystem::current_path() / "config_chat2.ini")
            << std::endl;
  ...
  try {
    ...
    size_t mysql_pool_size =
        std::max(16u, std::thread::hardware_concurrency() * 2);
    std::cout << "[ChatServer] MySQL pool size: " << mysql_pool_size
              << std::endl;
    mysqlPool->Init(mysql_url, mysql_user, mysql_passwd, mysql_schema,
                    mysql_pool_size);
    ...
    std::thread redis_sub_thread([]() {
      ...
      while (!bstop.load()) {
        ...
        if (!redis_guard) {
          std::cout << "[FriendNotify][Chat][Redis] connect failed host="
                    << host << ":" << port << std::endl;
        ...
        if (!passwd.empty()) {
          redisReply *auth =
              (redisReply *)redisCommand(ctx, "AUTH %s", passwd.c_str());
        ...
        try {
          ...
          redisReply *setname = (redisReply *)redisCommand(
              ctx, "CLIENT SETNAME %s", cname.c_str());
          if (setname)
            freeReplyObject(setname);
          std::cout << "[FriendNotify][Chat][Redis] SETNAME " << cname
                    << std::endl;
        } catch (...) {
        }
        ...
        redisReply *sub = (redisReply *)redisCommand(
            ctx, "SUBSCRIBE friend.apply friend.reply");
        if (!sub) {
          std::cout << "[FriendNotify][Chat][Redis] SUBSCRIBE send failed"
                    << std::endl;
          //  修复：移除手动redisFree，由RAII自动管理
          std::this_thread::sleep_for(std::chrono::seconds(2));
          continue;
        ...
        freeReplyObject(sub);
        std::cout << "[FriendNotify][Chat][Redis] SUBSCRIBED channels: "
                     "friend.apply, friend.reply"
                  << std::endl;
        // 订阅后进入读取循环
        ...
        while (!bstop.load()) {
          ...
          if (rc != REDIS_OK) {
            // 可能是超时或暂时无数据：hiredis 在超时/无数据时也会设置
            // err/errstr
            std::string es = ctx->errstr ? ctx->errstr : "";
            bool is_soft_timeout =
                (ctx->err == REDIS_ERR_TIMEOUT) ||
                (es.find("timed out") != std::string::npos) ||
                (es.find("Resource temporarily unavailable") !=
                 std::string::npos) ||
            ...
            if (is_soft_timeout || es.empty()) {
              ...
              if ((idle_ticks % 100) == 0) {
                std::cout
                    << "[FriendNotify][Chat][Redis] idle tick (no message)"
                    << std::endl;
              }
            ...
            // 确认为真实错误，记录并重连
            std::cout << "[FriendNotify][Chat][Redis] redisGetReply error, err="
                      << ctx->err << " errstr=" << es << " reconnecting..."
                      << std::endl;
            break;
          }
          std::unique_ptr<redisReply, void (*)(void *)> reply((redisReply *)r,
                                                              freeReplyObject);
          if (!reply)
            continue;
          ...
          if (reply->type == REDIS_REPLY_ARRAY) {
            ...
            std::string kind;
            if (elems >= 1 && reply->element[0]->type == REDIS_REPLY_STRING &&
                reply->element[0]->str) {
            ...
            if (kind != "message") {
              std::cout << "[FriendNotify][Chat][Redis] kind="
                        << (kind.empty() ? std::string("<nil>") : kind)
                        << " elems=" << elems;
              if (elems >= 2 && reply->element[1]->type == REDIS_REPLY_STRING &&
                  reply->element[1]->str) {
            ...
            if (kind == "message" && elems >= 3 &&
              ...
              std::string payload(reply->element[2]->str,
                                  reply->element[2]->len);
              std::cout << "[FriendNotify][Chat][Redis] recv channel="
                        << channel << " payload=" << payload << std::endl;
              Json::Value obj;
              Json::Reader rd;
              bool ok = rd.parse(payload, obj);
              if (!ok || !obj.isObject()) {
                std::cout << "[FriendNotify][Chat][Redis] invalid json payload"
                          << std::endl;
                continue;
              }
              ...
              if (channel == "friend.apply") {
                ...
                if (sess) {
                  std::cout
                      << "[FriendNotify][Chat][Redis] send TCP 1021 to_uid="
                      << to_uid << std::endl;
                  sess->Send(payload, ID_NOTIFY_ADD_FRIEND_REQ);
                } else {
                  std::cout
                      << "[FriendNotify][Chat][Redis] to_uid offline, skip"
                      << std::endl;
                }
                ...
                if (sess) {
                  std::cout
                      << "[FriendNotify][Chat][Redis] send TCP 1022 from_uid="
                      << from_uid << std::endl;
                  sess->Send(payload, ID_NOTIFY_FRIEND_REPLY);
                } else {
                  std::cout
                      << "[FriendNotify][Chat][Redis] from_uid offline, skip"
                      << std::endl;
                }
              }
              continue;
            ...
            if (kind == "pmessage" && elems >= 4 &&
                ...
                reply->element[3]->str) {
              std::string channel(reply->element[2]->str,
                                  reply->element[2]->len);
              std::string payload(reply->element[3]->str,
                                  reply->element[3]->len);
              std::cout << "[FriendNotify][Chat][Redis] recv pmessage channel="
                        << channel << " payload=" << payload << std::endl;
            }
          } else {
            // 非数组类型的回复，记录类型码以便诊断
            std::cout << "[FriendNotify][Chat][Redis] unexpected reply type="
                      << reply->type << std::endl;
          }
        }
    ...
    try {
      ...
      std::cerr << "Invalid port string: " << port_str << " err=" << e.what()
                << std::endl;
      return -1;
    }
    ...
    // [FriendNotify]
    if (redis_sub_thread.joinable())
      redis_sub_thread.join();
