...
CServer::CServer(boost::asio::io_context &io_context, unsigned short port)
  ...
  // 验证端口号
  if (port == 0) {
    std::cerr << "ERROR: invalid port 0 passed to CServer. Aborting."
              << std::endl;
  ...
  try {
    ...
    _acceptor.set_option(
        boost::asio::socket_base::reuse_address(true)); // 允许地址重用
    _acceptor.bind(ep);
    _acceptor.listen(
        boost::asio::socket_base::max_listen_connections); // 开始监听

    auto ep_local = _acceptor.local_endpoint();
    std::cout << "Acceptor bound, local endpoint port: " << ep_local.port()
              << std::endl;
  } catch (const boost::system::system_error &e) {
    std::cerr << "Acceptor init failed, port=" << port << ", err=" << e.what()
              << std::endl;
    throw;
  }
...
void CServer::StartAccept() {
  ...
  std::shared_ptr<CSession> new_session =
      std::make_shared<CSession>(_io_context, this);
