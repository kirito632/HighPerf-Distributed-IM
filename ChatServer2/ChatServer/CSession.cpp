...
void CSession::Send(std::string msg, short msgid) {
  ...
  if (send_que_size > MAX_SENDQUE) {
    ...
    std::cout << "[FlowControl] Slow consumer detected for session "
              << _session_id << ", uid=" << _user_uid
              << ". Closing connection to prevent memory exhaustion."
              << std::endl;
  ...
  boost::asio::async_write(
      ...
      boost::asio::bind_executor(
          _strand,
          [self, out_msgid, out_len, uid](const boost::system::error_code &ec,
                                          std::size_t bytes_transferred) {
            if (!ec) {
              std::cout << "[TCP][Write] ok uid=" << uid
                        << " msgid=" << out_msgid << " body_len=" << out_len
                        << " bytes=" << bytes_transferred << std::endl;
            } else {
              std::cout << "[TCP][Write] fail uid=" << uid
                        << " msgid=" << out_msgid << " body_len=" << out_len
  ...
  asyncReadFull(HEAD_TOTAL_LEN, [self,
    ...
    try {
      ...
      if (msg_id < 0 || msg_id > MAX_LENGTH) {
        std::cout << "invalid msg_id is " << msg_id << " (must be 0-"
                  << MAX_LENGTH << ")" << std::endl;
        Close();
        _server->ClearSession(_session_id);
        return;
      ...
      if (msg_len > MAX_LENGTH) {
        std::cout << "msg_len too large: " << msg_len << " (max: " << MAX_LENGTH
                  << ")" << std::endl;
        Close();
        _server->ClearSession(_session_id);
        return;
      ...
      if (msg_len > MAX_PACKAGE_SIZE) {
        std::cout << "potential DoS attack: msg_len=" << msg_len << " exceeds "
                  << MAX_PACKAGE_SIZE << " bytes" << std::endl;
        Close();
        _server->ClearSession(_session_id);
        return;
      }
  ...
  try {
    if (!error) {
      ...
      if (!_send_que.empty()) {
        ...
        ::memcpy(&out_msgid, msgnode->_data, HEAD_ID_LEN);
        out_msgid =
            boost::asio::detail::socket_ops::network_to_host_short(out_msgid);
        short out_len = 0;
        ::memcpy(&out_len, msgnode->_data + HEAD_ID_LEN, HEAD_DATA_LEN);
        out_len =
            boost::asio::detail::socket_ops::network_to_host_short(out_len);
        int uid = _user_uid;
        ...
        boost::asio::async_write(
            _socket, boost::asio::buffer(msgnode->_data, msgnode->_total_len),
            boost::asio::bind_executor(
                _strand, [self, out_msgid, out_len,
                          uid](const boost::system::error_code &ec,
                               std::size_t bytes_transferred) {
                  if (!ec) {
                    std::cout << "[TCP][Write] ok uid=" << uid
                              << " msgid=" << out_msgid
