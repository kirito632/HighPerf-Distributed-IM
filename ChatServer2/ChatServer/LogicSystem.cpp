...
void LogicSystem::RegisterCallBacks() {
  ...
  _fun_callbacks[ID_GET_OFFLINE_MSG_REQ] =
      std::bind(&LogicSystem::GetOfflineMsgHandler, this, std::placeholders::_1,
                std::placeholders::_2, std::placeholders::_3);

  _fun_callbacks[ID_NOTIFY_TEXT_CHAT_MSG_RSP] =
      std::bind(&LogicSystem::OfflineMsgAckHandler, this, std::placeholders::_1,
                std::placeholders::_2, std::placeholders::_3);
}
...
void LogicSystem::LoginHandler(std::shared_ptr<CSession> session,
  ...
  std::cout << "[LoginHandler] recv uid=" << uid << " token=" << token
            << std::endl;
  ...
  std::cout << "[LoginHandler TEST] send success, uid=" << uid
            << " body=" << return_str << " msgid=" << MSG_CHAT_LOGIN_RSP
            << std::endl;
  session->Send(return_str, MSG_CHAT_LOGIN_RSP);
...
void LogicSystem::DealChatTextMsg(std::shared_ptr<CSession> session,
  ...
  if (!b_ip) {
    std::cout << "[TextChat][Route] redis miss key=" << to_ip_key
              << " -> no route (msg saved)" << std::endl;
    return;
  }
  ...
  std::transform(server_name.begin(), server_name.end(), server_name.begin(),
                 ::tolower);
  std::cout << "[TextChat][Route] to_ip=" << to_ip_value
            << " self=" << server_name << " same_server=" << std::boolalpha
            << (to_ip_value == server_name) << std::endl;
  ...
  if (to_ip_value == server_name) {
    ...
    if (to_sess) {
      ...
      std::cout << "[OfflineMsg] user " << touid
                << " is offline, saved message to redis key=" << offline_key
                << std::endl;
    }
    return;
  }
  ...
  std::cout << "[TextChat][Route] cross-server deliver via gRPC target="
            << to_ip_value << " fromuid=" << uid << " touid=" << touid
            << " msgs=" << arrays.size() << std::endl;
  auto rsp = ChatGrpcClient::GetInstance()->NotifyTextChatMsg(to_ip_value,
                                                              text_msg_req);

  // 如果RPC调用成功，但业务逻辑返回对方离线
  if (rsp.error() == ErrorCodes::RecipientOffline) {
    std::cout << "[TextChat][Route] gRPC target offline, saving to local redis"
              << std::endl;
...
void LogicSystem::GetOfflineMsgHandler(std::shared_ptr<CSession> session,
                                       const short &msg_id,
                                       const std::string &msg_data) {
  Json::Reader reader;
  Json::Value root;
  ...
  std::cout << "[OfflineMsg] recv get offline msg req, uid=" << uid
            << std::endl;
  ...
  AsyncDBPool::GetInstance()->PostTask([uid, offline_key, weak_sess]() {
    ...
    if (!shared_sess) {
      std::cout
          << "[OfflineMsg][Async] session expired, abort db query for uid="
          << uid << std::endl;
      return;
    }
    ...
    if (MysqlMgr::GetInstance()->GetUnreadChatMessages(uid, ids, db_payloads)) {
      std::cout << "[OfflineMsg][Async] get " << db_payloads.size()
                << " unread messages for uid=" << uid << std::endl;
      ...
      std::cout << "[OfflineMsg][Async] cleared redis cache key=" << offline_key
                << std::endl;
    }
  });
}

void LogicSystem::OfflineMsgAckHandler(std::shared_ptr<CSession> session,
                                       const short &msg_id,
                                       const std::string &msg_data) {
  Json::Reader reader;
  Json::Value root;
  ...
  std::cout << "[OfflineMsg][Ack] recv ack for uid=" << uid
            << " max_msg_id=" << max_msg_id << std::endl;
...
bool LogicSystem::GetBaseInfo(std::string base_key, int uid,
  ...
  if (b_base) {
    ...
    std::cout << "user login uid is " << userinfo->uid << " user name is "
              << userinfo->name << " user email is " << userinfo->email
              << " pwd is " << userinfo->pwd << std::endl;
  } else {
